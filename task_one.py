...
def generate_random_numbers(n:int=15)->List[int]:
    """produce n random numbers(default 15)"""
    i = 1
    result = []
    while i <= n:
        result.append(random.random(1,83))
        i += 1
    return result


def get_url(resource_id:int,resourse:str)->str:
...
@timeit
def main()->None:
    parser=argparse.ArgumentParser(prog="starwarsAPI",
                                   usage="Fetches resources from swapi.dev based"
                                   "on whatever arguments we provide",
                                   description="It uses random number generator and uses request library"
...
# we are creating an option to provide count
    parser.add_argument('-c','-count',
                        default=5,
                        help="count of characters to fetch datafrom")
    ...
    parser.add_argument('-r', '-resource',
                        ...
                        choices=["people","films","starships","vehicles","spices","planets"]
                        )
